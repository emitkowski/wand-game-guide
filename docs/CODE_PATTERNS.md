...
### Telemetry logging is a separate channel from error logging
**Context:** Any `game_guide.*` log event — deciding where it should go.
**Pattern:** Two channels, two audiences. Genuine failures (`Log::error(...)`, e.g. `game_guide.anthropic_api_error`, `game_guide.assistant_reply_failed`) stay on the default channel — the one an ops/alerting setup watches. Everything else — sync/business events that happened correctly and are worth measuring (`game_guide.message_recorded`, `game_guide.message_replay_deduplicated`, `game_guide.messages_synced`, `game_guide.assistant_reply_generated`, `game_guide.feature_flag_exposure`) — goes to a dedicated `Log::channel('game_guide_telemetry')` (`config/logging.php`), writing to its own daily file (`storage/logs/game_guide_telemetry-*.log`, 30-day retention) instead of the main app log. Where a duration is meaningful, it's included as `duration_ms` (or a specifically-named variant, e.g. `anthropic_duration_ms`) in the context array — timed around exactly the operation the event names (the locked write transaction for `message_recorded`, the Anthropic HTTP call for `assistant_reply_generated`, the whole history-fetch request for `messages_synced`), not the surrounding method.
**Reasoning:** High-volume, always-succeeding telemetry events would otherwise crowd out genuine errors in the same file/channel, making the error log noisy and harder to alert on — and conversely, an ops rotation/retention policy tuned for "keep errors around briefly, escalate fast" is the wrong policy for "keep sync volume/performance data around longer for trend analysis." Splitting the channel means either can be shipped, rotated, or alerted on independently without touching the other's code path.
**Anti-pattern:** Logging a successful, expected event at `Log::error()` level (or vice versa) to make it "more visible" — severity should reflect the event's actual nature, not its perceived importance; use the channel split for visibility/retention differences instead.
**Example:** `app/Actions/RecordConversationMessage.php`, `app/Actions/GenerateGameGuideReply.php`, `app/Http/Controllers/Api/V1/ConversationMessageController.php` (telemetry) vs. `app/Services/AnthropicService.php`, `app/Jobs/GenerateGameGuideReplyJob.php` (errors, unchanged default channel).
