...
|---|---|---|---|
| `chat-history-sync` | Gates the Game Guide chat history sync API (`/api/v1/conversations/{conversation}/messages`) for the phased rollout. Defined in `AppServiceProvider::boot()`, defaults active for local/dev. | 2026-07-24 | Once the phased rollout reaches full GA and confidence is high |
