...
| Game Guide chat | Conversation/message persistence, ordering, idempotency, history sync | `conversations`/`messages` tables, `App\Actions\RecordConversationMessage` (the only writer), `App\Actions\FetchConversationMessages` (the only reader), the two `Api\V1\ConversationMessageController` endpoints, `game-guide/Chat.vue` | AI reply generation itself — that's the Game Guide AI component below; this component only persists whatever body text it's handed, player- or assistant-authored |
